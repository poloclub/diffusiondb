...
# It's checking if the user has provided any arguments, and if they have, it sets the variables
# to the arguments.
if args.index:
    index = args.index
if args.range:
...
if args.index and args.range and args.output and args.unzip is None:    # if no arguments are provided, set default behaviour
    index = 1
    range_max = 2000
    output = "images"
    unzip = False
...
def download(index=1, range_index=0, output=""):
    ...
    :param range_index: The number of files to download. If you want to download all files, set this to
    the number of files you want to download, defaults to 0 (optional)
    :param output: The directory to download the files to
    :return: A list of files to unzip
    """
...
# It's trying to download the file, and if it encounters an HTTPError, it prints the error.
                try:
...
# unzip. It would be a bad idea to put these together as the process is already lengthy.
...
def main(index=None, range_max=None, output=None, unzip=None):
    """
    `main` is a function that takes in an index, a range_max, an output, and an unzip, and if the user
    confirms that they have enough space, it downloads the files from the index to the output, and if
    unzip is true, it unzips them
    
    :param index: The index of the file you want to download
    :param range_max: The number of files to download
    :param output: The directory to download the files to
    :param unzip: If you want to unzip the files after downloading them, set this to True
...
# This is a common pattern in Python. It allows you to run the main function of your script by
# running the script through the interpreter. It also allows you to import the script into the
# interpreter
